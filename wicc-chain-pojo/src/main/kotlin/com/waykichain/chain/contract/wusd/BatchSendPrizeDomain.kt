...
class BatchSendPrizeDomain: WusdBaseDomain(){
    ...
    init {
        type = WusdConstants.WUSD_CONTRACT_TYPE_BATCH_SEND_PRIZE
    }
    ...
    override fun serialize():String{
        ...
        for(i in 0..(sendPrizeCount-1)){
            var transferModel: TransferModel =  sendPrizeList[i]
            result += ContractUtil.toHexString(transferModel.fromAddress)

            if(transferModel.toAddress.length != 34){
                throw Exception("The to address length should be 34. but the actual to address length is " + transferModel.toAddress.length)
            }
    ...
    override fun deserialize(contract:String):Int{
        ...
        var prizeContentHex = contract.substring(startIndex,endIndex)
        prizeContent = ContractUtil.hexToString(prizeContentHex)
        ...
        for(i in 1..sendPrizeCount){
            ...
            transferModel.fromAddress = ContractUtil.hexToString(fromHex)
            startIndex = endIndex
            endIndex = startIndex + 34*2
